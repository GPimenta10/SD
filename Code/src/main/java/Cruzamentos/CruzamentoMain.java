 ...
 * - Carregar a configuração do cruzamento do ficheiro config.json
...
public class CruzamentoMain {
    ...
    /**
     * Carrega a configuração de um cruzamento específico do ficheiro config.json.
     *
    ...
    private static JsonObject carregarConfiguracao(String nomeCruzamento) {
        ...
        if (config == null) {
            ...
            EnviarLogs.enviar(TipoLog.ERRO, "Cruzamento '" + nomeCruzamento + "' não existe no config.json");
            System.exit(1);
        }
